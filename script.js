...
searchBar.addEventListener('input', (e) => {
    ...
    const allContent = [...movieDatabase, ...seasonDatabase];
    ...
    if (filtered.length > 0) {
        ...
        filtered.slice(0, 10).forEach(item => {
            ...
            resultItem.innerHTML = `
                <h4>${item.title}</h4>
                <p>${item.episodes ? item.episodes + ' • ' : ''}${item.info}</p>
            `;
...
function showSearchedItem(item) {
    ...
    document.getElementById('character-img').src = 'images/thinking.png';
    ...
    setTimeout(() => {
        ...
        document.getElementById('result-description').textContent =
            (item.episodes ? item.episodes + ' • ' : '') + item.info;
